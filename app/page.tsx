...
export default function Home() {
  function handleLogin() {
    ...
    fetch('https://xmljewzdsqqsgselwubn.supabase.co/auth/v1/token?grant_type=password', {
      method: 'POST',
      headers: {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9f9f8' }}>
      <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '340px', border: '0.5px solid #e5e5e3', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <input id="password" type="password" placeholder="••••••••" onKeyDown={(e) => e.key === 'Enter' && handleLogin()} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '13px', color: '#111', outline: 'none' }} />
        </div>
