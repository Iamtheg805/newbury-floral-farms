...
const navItems = [
  { label: 'Overview', href: '/manager', active: true },
  { label: 'All Reps', href: '/manager/reps', active: false },
  { label: 'Manage Inventory', href: '/manager/inventory', active: false },
  { label: 'Commission Tiers', href: '/manager/tiers', active: false },
  { label: 'User Access', href: '/manager/users', active: false },
]
      ...
      <div style={{ width: '200px', background: '#ffffff', borderRight: '0.5px solid #e5e5e3', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div style={{ padding: '8px 0', borderTop: '0.5px solid #e5e5e3', marginTop: '8px' }}>
          ...
          {[
            ...
            <a key={item.label} href={item.href} style={{ display: 'block', padding: '9px 16px', fontSize: '12px', color: '#444', textDecoration: 'none' }}>
              {item.label}
            </a>
          ))}
        </div>
          ...
          <div>
            ...
            <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem' }}>
              ...
              <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Time', 'Rep', 'Customer', 'Flower', 'Total', 'Status'].map(h => (
          ...
          <div>
            ...
            <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem' }}>
              <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Rep', 'Revenue', 'Orders', 'Tier', 'Commission', 'Goal %', 'Progress'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '6px 8px', fontSize: '10px', fontWeight: '500', color: '#888', borderBottom: '0.5px solid #e5e5e3' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
        ...
        {showAdd && (
          <div style={{ background: '#f9f9f8', borderRadius: '10px', padding: '1rem', marginBottom: '12px', border: '0.5px solid #185FA5' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '8px' }}>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Flower name</label>
                <input value={newFlower.name} onChange={e => setNewFlower(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Peonies" style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Variety</label>
                <input value={newFlower.variety} onChange={e => setNewFlower(p => ({ ...p, variety: e.target.value }))} placeholder="e.g. Sarah Bernhardt" style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Unit</label>
                <select value={newFlower.unit} onChange={e => setNewFlower(p => ({ ...p, unit: e.target.value }))} style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }}>
                  <option value='bucket'>Bucket</option>
                  <option value='bunch'>Bunch</option>
                </select>
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Morning quantity</label>
                <input type="number" value={newFlower.qty} onChange={e => setNewFlower(p => ({ ...p, qty: parseInt(e.target.value) || 0 }))} style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Price per unit ($)</label>
                <input type="number" value={newFlower.price} onChange={e => setNewFlower(p => ({ ...p, price: parseFloat(e.target.value) || 0 }))} style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
            </div>
            ...
            <div key={f.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 70px 80px 30px', gap: '6px', alignItems: 'center', padding: '8px 0', borderBottom: '0.5px solid #f0f0ee' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '10px', color: '#888' }}>{f.variety} · {f.unit}</div>
                </div>
              </div>
...
function TiersTab({ reps }: { reps: typeof import('./page').default extends () => unknown ? never : { name: string; initials: string; bg: string; tc: string; revenue: number; orders: number; rate: string; confirmed: number; pending: number; goal: number }[] }) {
    ...
    <div>
      ...
      <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem', marginBottom: '10px' }}>
        ...
        {tiers.map((t, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 80px', gap: '8px', padding: '6px 0', borderBottom: '0.5px solid #f0f0ee', alignItems: 'center' }}>
            <input value={t.name} onChange={e => { const u = [...tiers]; u[i].name = e.target.value; setTiers(u) }} style={{ padding: '6px', borderRadius: '6px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
            <input type="number" value={t.min} onChange={e => { const u = [...tiers]; u[i].min = parseInt(e.target.value) || 0; setTiers(u) }} style={{ padding: '6px', borderRadius: '6px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
            <input type="number" value={t.max === 999999 ? '' : t.max} placeholder="No limit" onChange={e => { const u = [...tiers]; u[i].max = parseInt(e.target.value) || 999999; setTiers(u) }} style={{ padding: '6px', borderRadius: '6px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
            <input type="number" value={t.rate} onChange={e => { const u = [...tiers]; u[i].rate = parseInt(e.target.value) || 0; setTiers(u) }} style={{ padding: '6px', borderRadius: '6px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
          </div>
        ))}
      ...
      <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem' }}>
        ...
        <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['Rep', 'Revenue', 'Tier', 'Confirmed', 'Pending'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '6px 8px', fontSize: '10px', fontWeight: '500', color: '#888', borderBottom: '0.5px solid #e5e5e3' }}>{h}</th>
              ))}
            </tr>
          </thead>
...
function UsersTab() {
  const [users, setUsers] = useState([
    { name: 'Rosa Martinez', email: 'rosa.m@newburyfloral.com', role: 'manager', last: 'Today 8:02 AM', status: 'active' },
    { name: 'Jake Rivera', email: 'jake.r@newburyfloral.com', role: 'rep', last: 'Today 9:15 AM', status: 'active' },
    { name: 'Sarah Lee', email: 'sarah.l@newburyfloral.com', role: 'rep', last: 'Today 8:44 AM', status: 'active' },
    { name: 'Mike Kim', email: 'mike.k@newburyfloral.com', role: 'rep', last: 'Today 10:01 AM', status: 'active' },
    { name: 'Dana Perez', email: 'dana.p@newburyfloral.com', role: 'rep', last: 'Yesterday', status: 'active' },
    { name: 'Tom Walsh', email: 'tom.w@newburyfloral.com', role: 'rep', last: 'Today 7:58 AM', status: 'active' },
  ])
  ...
  function inviteUser() {
    ...
    setUsers(prev => [...prev, { name: newUser.name, email: newUser.email, role: newUser.role, last: 'Never', status: 'active' }])
    ...
    <div>
          ...
          <div style={{ background: '#f9f9f8', borderRadius: '10px', padding: '1rem', marginBottom: '12px', border: '0.5px solid #185FA5' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: '8px', marginBottom: '10px' }}>
              <div>
                ...
                <input value={newUser.name} onChange={e => setNewUser(p => ({ ...p, name: e.target.value }))} placeholder="e.g. John Smith" style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Email</label>
                <input value={newUser.email} onChange={e => setNewUser(p => ({ ...p, email: e.target.value }))} placeholder="email@newburyfloral.com" style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '3px' }}>Role</label>
                <select value={newUser.role} onChange={e => setNewUser(p => ({ ...p, role: e.target.value }))} style={{ width: '100%', padding: '7px', borderRadius: '8px', border: '0.5px solid #e5e5e3', fontSize: '12px', color: '#111' }}>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.email}>
                ...
                <td style={{ padding: '9px 8px', borderBottom: '0.5px solid #f0f0ee' }}>
                  <span style={{ background: u.role === 'manager' ? '#FAEEDA' : '#E6F1FB', color: u.role === 'manager' ? '#633806' : '#185FA5', padding: '2px 7px', borderRadius: '99px', fontSize: '10px', fontWeight: '500' }}>{u.role}</span>
                </td>
                <td style={{ padding: '9px 8px', color: '#888', borderBottom: '0.5px solid #f0f0ee' }}>{u.last}</td>
                <td style={{ padding: '9px 8px', borderBottom: '0.5px solid #f0f0ee' }}>
                  <span style={{ background: '#EAF3DE', color: '#3B6D11', padding: '2px 7px', borderRadius: '99px', fontSize: '10px', fontWeight: '500' }}>active</span>
                </td>
                <td style={{ padding: '9px 8px', borderBottom: '0.5px solid #f0f0ee' }}>
                  {u.role !== 'manager' && <button onClick={() => removeUser(u.email)} style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: '#A32D2D', fontSize: '11px' }}>Remove</button>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem' }}>
        ...
        <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['Permission', 'Sales Rep', 'Manager'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '6px 8px', fontSize: '10px', fontWeight: '500', color: '#888', borderBottom: '0.5px solid #e5e5e3' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={label as string}>
                ...
                <td style={{ padding: '8px', borderBottom: '0.5px solid #f0f0ee' }}>
                  <span style={{ background: rep ? '#EAF3DE' : '#FCEBEB', color: rep ? '#3B6D11' : '#A32D2D', padding: '2px 7px', borderRadius: '99px', fontSize: '10px', fontWeight: '500' }}>{rep ? 'Yes' : 'No'}</span>
                </td>
                <td style={{ padding: '8px', borderBottom: '0.5px solid #f0f0ee' }}>
                  <span style={{ background: mgr ? '#EAF3DE' : '#FCEBEB', color: mgr ? '#3B6D11' : '#A32D2D', padding: '2px 7px', borderRadius: '99px', fontSize: '10px', fontWeight: '500' }}>{mgr ? 'Yes' : 'No'}</span>
                </td>
              </tr>
            ))}
          </tbody>
