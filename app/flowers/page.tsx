...
export default function Flowers() {
  ...
  useEffect(() => {
    ...
    if (repId) {
      fetch(`/api/customers/list?rep_id=${repId}`).then(r => r.json()).then(data => {
  ...
  async function doSell() {
    if (!selected) return
    if (qty > selected.current_stock) { setFeedback(`Not enough stock. Only ${selected.current_stock} available.`); return }
    if (!customer) { setFeedback('Please select a customer.'); return }
    try {
      const res = await fetch('/api/flowers/sell', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: selected.id, qty }) })
    ...
    } catch { setFeedback('Could not update stock.') }
  }
        ...
        {selected && (
          <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              ...
              <button onClick={() => { setSelected(null); setFeedback('') }} style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: '16px', color: '#888' }}>x</button>
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button onClick={doSell} style={{ padding: '7px 16px', background: '#185FA5', color: 'white', border: 'none', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>Sell & update</button>
            </div>
        ...
        <div style={{ background: 'white', border: '0.5px solid #e5e5e3', borderRadius: '12px', padding: '1rem' }}>
          ...
          {log.length === 0 ? (
            ...
            <div key={i} style={{ display: 'flex', gap: '8px', fontSize: '11px', padding: '5px 0', borderBottom: '0.5px solid #f0f0ee' }}>
              ...
              <span style={{ color: '#888' }}>{l.qty} {l.unit}s to {l.customer}</span>
            </div>
          ))}
        </div>
      </div>
